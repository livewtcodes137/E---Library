...
export default class TransactionScreen extends Component {
	...
	handleTransaction = async () => {
		...
		if (!transactionType) {
			...
			this.setState({ bookId: '', studentId: '' });
			// For Android users only
			ToastAndroid.show("Student id is inncorrect or you have reached the maximum amount of books to be issued", ToastAndroid.SHORT);
			Alert.alert("Student id is inncorrect or you have reached the maximum amount of books to be issued");
	...
	checkStudentEligibilityForBookIssue = async (studentId) => {
		...
		var studentEligiblility = '';

		if (studentRef.docs.length == 0) {
			 studentEligiblility= false;
		} else {
			studentRef.forEach((doc) => {
				//if the book is available then transaction type will be issue
				// otherwise it will be return
				studentEligiblility = doc.data().number_of_books_issued < 3 ? 'true' : 'false';
			});
		}
         
		return studentEligiblility ; 
